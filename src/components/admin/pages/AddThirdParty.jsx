...
const AddThirdParty = () => {
  ...
  const [selectedVegetables, setSelectedVegetables] = useState([]); // Change to empty array
  const [availableVegetables, setAvailableVegetables] = useState([]); // Add state for available vegetables
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.message) {
        ...
        setError(err.message || 'Failed to create third party. Please try again.');
      }
    } finally {
      setLoading(false);
    }
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[#D0E0DB] p-6">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-6 border-t border-gray-200">
              ...
              >
                {loading ? 'Adding...' : 'Add Third Party'}
              </button>
              <button
                type="button"
                onClick={() => navigate('/third-party')}
