...
const ReportThirdPartyView = () => {
    ...
    const fetchThirdPartyDetails = async () => {
        try {
            ...
            // Fetch all third parties
            const thirdPartiesResponse = await getAllThirdParties();
            if (thirdPartiesResponse?.data) {
                const foundThirdParty = thirdPartiesResponse.data.find(tp => tp.tpid == thirdPartyId);
                if (foundThirdParty) {
                    setThirdParty(foundThirdParty);
                } else {
                    console.error('Third Party not found with ID:', thirdPartyId);
                }
            }
            ...
            if (ordersResponse?.data) {
                ...
                // Process each order to find assignments for this third party
                for (const order of orders) {
                    try {
                        ...
                        if (assignmentRes?.data?.product_assignments) {
                            ...
                            // Filter assignments for this third party
                            const thirdPartyAssignments = assignments.filter(
                                assignment => assignment.entityType === 'thirdParty' && assignment.entityId == thirdPartyId
                            );
                            ...
                            if (thirdPartyAssignments.length > 0) {
                                ...
                                // Enrich assignments with correct quantity and price
                                const enrichedAssignments = thirdPartyAssignments.map(assignment => {
                                    ...
                                    // Get price from stage4 product rows
                ...
                setThirdPartyOrders(processedOrders);
            }
        } catch (error) {
    ...
    const handleExportOrderPDF = (order, assignments, totalAmount) => {
        ...
        doc.text('THIRD PARTY ORDER DETAILS', 105, 12, { align: 'center' });
        doc.setFontSize(12);
        doc.setFont(undefined, 'normal');
        doc.text(`${cleanText(order.oid)} - ${cleanText(thirdParty.third_party_name)}`, 105, 22, { align: 'center' });

        // Order Info
        doc.setTextColor(0, 0, 0);
        doc.autoTable({
            startY: 35,
            head: [['Order ID', 'Third Party Name', 'Order Date', 'Total Amount']],
            body: [[
                cleanText(order.oid),
                cleanText(thirdParty.third_party_name),
                fullDate,
        ...
        doc.setFontSize(13);
        doc.setFont(undefined, 'bold');
        doc.text("Products Assigned", 16, finalY + 4);
        doc.setFont(undefined, 'normal');
    ...
    const handleExportOrderExcel = (order, assignments, totalAmount) => {
        ...
        // Title
        allRows.push([cell('THIRD PARTY ORDER DETAILS', 'title'), '', '', '', '']);
        merges.push({ s: { r: currentRow, c: 0 }, e: { r: currentRow, c: 4 } });
        currentRow++;
        allRows.push([cell(`${order.oid} - ${thirdParty.third_party_name}`, 'title'), '', '', '', '']);
        merges.push({ s: { r: currentRow, c: 0 }, e: { r: currentRow, c: 4 } });
        currentRow++;
        allRows.push([]);
        currentRow++;
        ...
        currentRow++;
        allRows.push([
            cell('Third Party:', 'bold'),
            cell(thirdParty.third_party_name),
            cell('Total:', 'bold'),
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Third Party Order");
        XLSX.writeFile(wb, `ThirdParty_Order_${thirdParty.third_party_name}_${order.oid}.xlsx`);
    };

    // Filter orders by date range and payment status
    const getFilteredOrders = () => {
        let filtered = thirdPartyOrders;
    ...
    const handleExportPDF = () => {
        if (!thirdParty || filteredOrders.length === 0) return;
        ...
        doc.setFontSize(24);
        doc.setFont(undefined, 'bold');
        doc.text('THIRD PARTY REPORT', 105, 12, { align: 'center' });
        doc.setFontSize(12);
        doc.setFont(undefined, 'normal');
        doc.text(cleanText(thirdParty.third_party_name), 105, 22, { align: 'center' });

        // Third Party Info Card
        doc.setTextColor(0, 0, 0);
        doc.autoTable({
            startY: 35,
            head: [['Third Party ID', 'Phone', 'Total Orders', 'Total Amount']],
            body: [[
                cleanText(thirdParty.tpid),
                cleanText(thirdParty.phone || 'N/A'),
        ...
        doc.save(`ThirdParty_Report_${thirdParty.third_party_name}_${new Date().toISOString().split('T')[0]}.pdf`);
    };
    ...
    const handleExportExcel = () => {
        ...
        // Title
        allRows.push([cell('THIRD PARTY REPORT', 'title'), '', '', '', '']);
        merges.push({ s: { r: currentRow, c: 0 }, e: { r: currentRow, c: 4 } });
        currentRow++;
        allRows.push([cell(thirdParty.third_party_name, 'title'), '', '', '', '']);
        merges.push({ s: { r: currentRow, c: 0 }, e: { r: currentRow, c: 4 } });
        currentRow++;
        allRows.push([]);
        currentRow++;
        ...
        // Third Party Info
        allRows.push([
            cell('Third Party ID:', 'bold'),
            cell(thirdParty.tpid),
            cell('Phone:', 'bold'),
            cell(thirdParty.phone || 'N/A'),
            ''
        ]);
        currentRow++;
        allRows.push([]);
        ...
        XLSX.writeFile(wb, `ThirdParty_Report_${thirdParty.third_party_name}_${new Date().toISOString().split('T')[0]}.xlsx`);
    };

    if (loading) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-[#0D8568] text-xl">Loading third party details...</div>
            </div>
        );
    }
    ...
    if (!thirdParty) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-red-600 text-xl">Third Party not found</div>
            </div>
        );
    }
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-6 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button
                            onClick={() => navigate('/reports/third-party')}
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-[#0D5C4D]">Third Party Details</h1>
                            <p className="text-[#6B8782]">View detailed information about this third party</p>
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
                    <h2 className="text-2xl font-bold text-[#0D5C4D] mb-4">Third Party Information</h2>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div>
                            <p className="text-sm text-[#6B8782] mb-1">Third Party ID</p>
                            <p className="text-lg font-semibold text-[#0D5C4D]">{thirdParty.tpid || '-'}</p>
                        </div>
                        <div>
                            <p className="text-sm text-[#6B8782] mb-1">Name</p>
                            <p className="text-lg font-semibold text-[#0D5C4D]">{thirdParty.third_party_name || '-'}</p>
                        </div>
                        <div>
                            <p className="text-sm text-[#6B8782] mb-1">Phone Number</p>
                            <p className="text-lg font-semibold text-[#0D5C4D]">{thirdParty.phone || '-'}</p>
                        </div>
                        <div>
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
                    ...
                    {(fromDate || toDate || paymentStatusFilter !== 'all') && (
                        <div className="mt-4 text-sm text-[#0D8568]">
                            Showing {filteredOrders.length} of {thirdPartyOrders.length} orders
                        </div>
                    )}
                </div>
                    ...
                    <div className="p-6">
                        {filteredOrders.length === 0 ? (
                            <div className="text-center py-12 text-[#6B8782]">
                                No orders found {(fromDate || toDate) ? 'for selected date range' : 'for this third party'}
                            </div>
                        ) : (
                            <>
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-4 py-4">
                                                            <div className="flex items-center gap-2">
                                                                <button
                                                                    onClick={() => navigate(`/admin/report-third-party/${thirdPartyId}/order/${order.oid}`)}
                                                                    className="p-2 bg-[#0D8568] hover:bg-[#0a6354] text-white rounded-lg transition-colors"
                                                                    title="View Third Party's Order Details"
                                                                >
                                                                    <Eye size={16} />
                                                                </button>
                                                                <button
...
export default ReportThirdPartyView;
