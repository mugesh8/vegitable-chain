...
const ReportFarmerOrderView = () => {
    const { farmerId, orderId } = useParams();
    const navigate = useNavigate();
    const [farmer, setFarmer] = useState(null);
    const [order, setOrder] = useState(null);
    const [farmerAssignments, setFarmerAssignments] = useState([]);
            ...
            if (ordersResponse?.data) {
                ...
                if (foundOrder) {
                    ...
                    if (assignmentRes?.data?.product_assignments) {
                        ...
                        // Filter assignments for this farmer only
                        const farmerOnly = assignments.filter(
                            assignment => assignment.entityType === 'farmer' && assignment.entityId == farmerId
                        );



                        // Debug: log ALL assignment data to see what's available
                        console.log('=== FULL ASSIGNMENT RESPONSE ===');
                        console.log('Full assignmentRes.data:', assignmentRes.data);
                        console.log('Farmer assignments:', farmerOnly);
                        ...
                        console.log('Stage 4 product rows:', stage4ProductRows);

                        // Enrich assignments with data from order items and stage4
                        const enrichedAssignments = farmerOnly.map(assignment => {
                            ...
                            // Get price from stage4 product rows (farmer's actual price)
                            let price = assignment.price || 0;
                            if (!price || price === 0) {
                                // Find the matching stage4 entry for this farmer and product
                                ...
                                if (stage4Entry) {
                                    console.log('Found stage4 entry for', cleanAssignmentProduct, stage4Entry);
                        ...
                        setFarmerAssignments(enrichedAssignments);
                    }
                }
            }
        } catch (error) {
    ...
    // Calculate total amount for this farmer in this order
    const calculateTotalAmount = () => {
        return farmerAssignments.reduce((sum, assignment) => {
    ...
    const handleExportPDF = () => {
        ...
        const productsBody = farmerAssignments.map(assignment => [
    ...
    const handleExportExcel = () => {
        ...
        farmerAssignments.forEach(assignment => {
    ...
    if (!farmer || !order) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-red-600 text-xl">Order or farmer not found</div>
            </div>
        );
    }
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-6 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button
                            onClick={() => navigate(`/admin/report-farmer/${farmerId}`)}
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-[#0D5C4D]">Farmer Order Details</h1>
                            <p className="text-[#6B8782]">{order.oid} - {farmer.farmer_name}</p>
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        ...
                        <div>
                            <p className="text-sm text-[#6B8782] mb-1">Farmer's Total Amount</p>
                ...
                <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
                    <div className="bg-[#0D8568] text-white px-6 py-4">
                        <h2 className="text-xl font-bold">Products Assigned to {farmer.farmer_name}</h2>
                    </div>
                    <div className="p-6">
                        {farmerAssignments.length === 0 ? (
                            <div className="text-center py-12 text-[#6B8782]">
                                No products assigned to this farmer in this order
                            </div>
                        ) : (
                            <div className="flex flex-col gap-4">
                                <div className="overflow-x-auto">
                                    <table className="w-full">
                                        ...
                                        <tbody>
                                            {(() => {
                                                const totalPages = Math.ceil(farmerAssignments.length / itemsPerPage);
                                                const startIndex = (currentPage - 1) * itemsPerPage;
                                                const endIndex = startIndex + itemsPerPage;
                                                const paginatedAssignments = farmerAssignments.slice(startIndex, endIndex);
                                ...
                                {/* Pagination Controls */}
                                {Math.ceil(farmerAssignments.length / itemsPerPage) > 1 && (
                                    <div className="flex flex-col md:flex-row items-center justify-between mt-6 pt-6 border-t border-[#D0E0DB] gap-4">
                                        <p className="text-sm text-[#6B8782]">
                                            ...
                                            <span className="font-semibold text-[#0D5C4D]">
                                                {Math.min(currentPage * itemsPerPage, farmerAssignments.length)}
                                            </span>{' '}
                                            of <span className="font-semibold text-[#0D5C4D]">{farmerAssignments.length}</span> items
                                        </p>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex items-center gap-1">
                                                {[...Array(Math.ceil(farmerAssignments.length / itemsPerPage))].map((_, i) => (
                                                    <button
                                                        key={i + 1}
                                            ...
                                            <button
                                                onClick={() => setCurrentPage(prev => Math.min(prev + 1, Math.ceil(farmerAssignments.length / itemsPerPage)))}
                                                disabled={currentPage === Math.ceil(farmerAssignments.length / itemsPerPage)}
                                                className={`p-2 rounded-lg border border-[#D0E0DB] transition-all ${currentPage === Math.ceil(farmerAssignments.length / itemsPerPage)
                                                    ? 'text-gray-300 cursor-not-allowed'
                                                    : 'text-[#0D5C4D] hover:bg-[#D4F4E8] hover:border-[#0D8568]'
                                                    }`}
                                            >
...
export default ReportFarmerOrderView;
