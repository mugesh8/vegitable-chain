...
const ReportThirdPartyOrderView = () => {
    const { thirdPartyId, orderId } = useParams();
            ...
            if (ordersResponse?.data) {
                ...
                if (foundOrder) {
                    ...
                    if (assignmentRes?.data?.product_assignments) {
                        ...
                        // Filter assignments for this third party only
                        const thirdPartyOnly = assignments.filter(
                        ...
                        const enrichedAssignments = thirdPartyOnly.map(assignment => {
                            ...
                            // Get price from stage4 product rows (third party's actual price)
                            let price = assignment.price || 0;
                            if (!price || price === 0) {
                                // Find the matching stage4 entry for this third party and product
                        ...
                        setThirdPartyAssignments(enrichedAssignments);
                    }
                }
            }
        } catch (error) {
    ...
    // Calculate total amount for this third party in this order
    const calculateTotalAmount = () => {
        return thirdPartyAssignments.reduce((sum, assignment) => {
    ...
    const handleExportPDF = () => {
        ...
        const productsBody = thirdPartyAssignments.map(assignment => [
    ...
    if (!thirdParty || !order) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-red-600 text-xl">Order or third party not found</div>
            </div>
        );
    }
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-6 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button
                            onClick={() => navigate(`/admin/report-third-party/${thirdPartyId}`)}
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-[#0D5C4D]">Third Party Order Details</h1>
                            <p className="text-[#6B8782]">{order.oid} - {thirdParty.third_party_name}</p>
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        ...
                        <div>
                            <p className="text-sm text-[#6B8782] mb-1">Third Party Name</p>
                ...
                <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
                    <div className="bg-[#0D8568] text-white px-6 py-4">
                        <h2 className="text-xl font-bold">Products Assigned to {thirdParty.third_party_name}</h2>
                    </div>
                    <div className="p-6">
                        {thirdPartyAssignments.length === 0 ? (
                            <div className="text-center py-12 text-[#6B8782]">
                                No products assigned to this third party in this order
                            </div>
                        ) : (
                            <div className="flex flex-col gap-4">
                                <div className="overflow-x-auto">
                                    <table className="w-full">
                                        ...
                                        <tbody>
                                            {(() => {
                                                const totalPages = Math.ceil(thirdPartyAssignments.length / itemsPerPage);
                                                const startIndex = (currentPage - 1) * itemsPerPage;
                                                const endIndex = startIndex + itemsPerPage;
                                                const paginatedAssignments = thirdPartyAssignments.slice(startIndex, endIndex);
                                ...
                                {/* Pagination Controls */}
                                {Math.ceil(thirdPartyAssignments.length / itemsPerPage) > 1 && (
                                    <div className="flex flex-col md:flex-row items-center justify-between mt-6 pt-6 border-t border-[#D0E0DB] gap-4">
                                        <p className="text-sm text-[#6B8782]">
                                            ...
                                            <span className="font-semibold text-[#0D5C4D]">
                                                {Math.min(currentPage * itemsPerPage, thirdPartyAssignments.length)}
                                            </span>{' '}
                                            of <span className="font-semibold text-[#0D5C4D]">{thirdPartyAssignments.length}</span> items
                                        </p>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex items-center gap-1">
                                                {[...Array(Math.ceil(thirdPartyAssignments.length / itemsPerPage))].map((_, i) => (
                                                    <button
                                                        key={i + 1}
                                            ...
                                            <button
                                                onClick={() => setCurrentPage(prev => Math.min(prev + 1, Math.ceil(thirdPartyAssignments.length / itemsPerPage)))}
                                                disabled={currentPage === Math.ceil(thirdPartyAssignments.length / itemsPerPage)}
                                                className={`p-2 rounded-lg border border-[#D0E0DB] transition-all ${currentPage === Math.ceil(thirdPartyAssignments.length / itemsPerPage)
                                                    ? 'text-gray-300 cursor-not-allowed'
                                                    : 'text-[#0D5C4D] hover:bg-[#D4F4E8] hover:border-[#0D8568]'
                                                    }`}
                                            >
...
export default ReportThirdPartyOrderView;
